...
| # | 规则 | 说明 |
|---|---|---|
| 1 | **会话备份必须默认保存到 `/sdcard/Download/DSHA/<当前会话编号>/` 下的两个子文件夹** | 隔离箱启动自动备份（**程序级完整备份**：整个 `.dsh` 打包，同 DSHA 工作区备份，程序损坏时用）→ `auto-backups/`；日志备份（backup.py 显式/finalize/safe_restart）→ `session-logs/`；父目录可用 `DSH_BACKUP_ROOT` 覆盖 |
